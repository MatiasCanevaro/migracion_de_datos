...
detalle_pago_id DECIMAL(18,0) null,
...
ticket_fecha_y_hora DATETIME not null
)
...
ALTER TABLE FURIOUS_QUERYING.ITEM add constraint pk_item primary key (producto_id, ticket_numero, tipo_comprobante_id,sucursal_nombre, ticket_fecha_y_hora) 
ALTER TABLE FURIOUS_QUERYING.ITEM_X_PROMOCION add constraint pk_item_x_promocion primary key (producto_id,ticket_numero,codigo_promocion, tipo_comprobante_id, sucursal_nombre, ticket_fecha_y_hora) 
ALTER TABLE FURIOUS_QUERYING.SUBCATEGORIA add constraint pk_subcategoria primary key (id,categoria_id) 
ALTER TABLE FURIOUS_QUERYING.CAJA add constraint pk_caja primary key (numero,sucursal_nombre)
ALTER TABLE FURIOUS_QUERYING.TICKET add constraint pk_ticket primary key (numero, tipo_comprobante_id,sucursal_nombre, fecha_y_hora)
ALTER TABLE FURIOUS_QUERYING.DESCUENTO_POR_PAGO add constraint pk_descuento_por_pago primary key(descuento_codigo,pago_numero) 
--===================================================FOREIGN KEYS=============================================================
...
ALTER TABLE FURIOUS_QUERYING.PAGO add constraint fk_ticket foreign key (ticket_numero, tipo_comprobante_id,sucursal_nombre, ticket_fecha_y_hora) references FURIOUS_QUERYING.TICKET (numero, tipo_comprobante_id, sucursal_nombre, fecha_y_hora)
...
ALTER TABLE FURIOUS_QUERYING.DESCUENTO_POR_PAGO add constraint fk_descuento_codigo foreign key (descuento_codigo) references FURIOUS_QUERYING.DESCUENTO (codigo)
...
ALTER TABLE FURIOUS_QUERYING.TICKET add constraint fk_caja foreign key (caja_numero,sucursal_nombre) references FURIOUS_QUERYING.CAJA (numero,sucursal_nombre)
...
ALTER TABLE FURIOUS_QUERYING.ENVIO add constraint fk_ticket_numero_por_envio foreign key (ticket_numero, tipo_comprobante_id,sucursal_nombre, ticket_fecha_y_hora) references FURIOUS_QUERYING.TICKET (numero, tipo_comprobante_id,sucursal_nombre, fecha_y_hora )
...
ALTER TABLE FURIOUS_QUERYING.ITEM add constraint fk_ticket_numero_por_item foreign key (ticket_numero, tipo_comprobante_id, sucursal_nombre,ticket_fecha_y_hora) references FURIOUS_QUERYING.TICKET (numero, tipo_comprobante_id,sucursal_nombre,fecha_y_hora)
...
ALTER TABLE FURIOUS_QUERYING.ITEM_X_PROMOCION add constraint fk_item foreign key (producto_id,ticket_numero, tipo_comprobante_id,sucursal_nombre, ticket_fecha_y_hora) references FURIOUS_QUERYING.ITEM (producto_id,ticket_numero, tipo_comprobante_id,sucursal_nombre, ticket_fecha_y_hora)
...
ALTER TABLE FURIOUS_QUERYING.CAJA add constraint fk_sucursal_nombre_por_caja foreign key (sucursal_nombre) references FURIOUS_QUERYING.SUCURSAL (nombre)
...
AS BEGIN
INSERT INTO FURIOUS_QUERYING.CAJA(numero,tipo_caja_id, sucursal_nombre) SELECT DISTINCT CAJA_NUMERO, t.id, SUCURSAL_NOMBRE 
                                                       FROM gd_esquema.Maestra
...
INSERT INTO FURIOUS_QUERYING.TICKET(numero,fecha_y_hora, subtotal_productos, descuento_promociones_total, descuento_medio_de_pago_total, 
                            ...
                            JOIN FURIOUS_QUERYING.TIPO_COMPROBANTE tc ON tc.tipo_comprobante = TICKET_TIPO_COMPROBANTE
...
INSERT INTO FURIOUS_QUERYING.ENVIO(fecha_programada,fecha_entrega,hora_inicio_programada,hora_fin_programada,costo,
                                    estado_envio_id,ticket_numero, cliente_id, tipo_comprobante_id, ticket_fecha_y_hora, sucursal_nombre)
                            SELECT DISTINCT ENVIO_FECHA_PROGRAMADA, ENVIO_FECHA_ENTREGA, ENVIO_HORA_INICIO, ENVIO_HORA_FIN,ENVIO_COSTO, 
                            ee.id, TICKET_NUMERO, c.id, tp.id, TICKET_FECHA_HORA, SUCURSAL_NOMBRE
                            FROM gd_esquema.Maestra
                            JOIN FURIOUS_QUERYING.ESTADO_ENVIO ee ON ee.estado = ENVIO_ESTADO
                            JOIN FURIOUS_QUERYING.CLIENTE c ON c.dni = CLIENTE_DNI
							JOIN FURIOUS_QUERYING.TIPO_COMPROBANTE tp ON tp.tipo_comprobante = TICKET_TIPO_COMPROBANTE
...
INSERT INTO FURIOUS_QUERYING.DETALLE_PAGO(cliente_id, tarjeta_numero, cuotas)
                            SELECT c.id, ma.PAGO_TARJETA_NRO, ma.PAGO_TARJETA_CUOTAS  FROM gd_esquema.Maestra ma
							JOIN gd_esquema.Maestra M ON (ma.TICKET_NUMERO = m.TICKET_NUMERO AND m.CLIENTE_DNI is not null)
							JOIN FURIOUS_QUERYING.CLIENTE c ON (c.dni = m.CLIENTE_DNI)
							WHERE ma.PAGO_FECHA IS NOT NULL and ma.PAGO_TARJETA_NRO is not null	
END
...
INSERT INTO FURIOUS_QUERYING.PAGO(fecha_y_hora,detalle_pago_id,medio_de_pago_codigo,ticket_numero,importe,descuento_aplicado, 
								tipo_comprobante_id, sucursal_nombre, ticket_fecha_y_hora)
                            SELECT DISTINCT PAGO_FECHA, dp.id ,mp.codigo, TICKET_NUMERO ,PAGO_IMPORTE,PAGO_DESCUENTO_APLICADO,tp.id, 
							SUCURSAL_NOMBRE, TICKET_FECHA_HORA
                            FROM gd_esquema.Maestra
                            LEFT JOIN FURIOUS_QUERYING.DETALLE_PAGO dp ON dp.tarjeta_numero = PAGO_TARJETA_NRO
...
AS BEGIN
INSERT INTO FURIOUS_QUERYING.ITEM(producto_id, ticket_numero, cantidad, precio_unitario, total, tipo_comprobante_id, ticket_fecha_y_hora, sucursal_nombre)
                            SELECT DISTINCT p.id, TICKET_NUMERO, sum(TICKET_DET_CANTIDAD),  TICKET_DET_PRECIO, sum(TICKET_DET_TOTAL), tp.id, TICKET_FECHA_HORA, SUCURSAL_NOMBRE
                            FROM gd_esquema.Maestra
                            JOIN FURIOUS_QUERYING.MARCA m ON m.marca = PRODUCTO_MARCA
                            JOIN FURIOUS_QUERYING.PRODUCTO p ON p.nombre = PRODUCTO_NOMBRE AND p.marca_id = m.id 
							JOIN FURIOUS_QUERYING.TIPO_COMPROBANTE tp ON tp.tipo_comprobante = TICKET_TIPO_COMPROBANTE
                            AND p.subcategoria_id = PRODUCTO_SUB_CATEGORIA AND p.categoria_id = PRODUCTO_CATEGORIA
                            WHERE TICKET_NUMERO IS NOT NULL
							group by p.id, TICKET_NUMERO, SUCURSAL_NOMBRE, tp.id, TICKET_FECHA_HORA,TICKET_DET_PRECIO
END

GO
CREATE PROCEDURE FURIOUS_QUERYING.MIGRAR_ITEM_X_PROMOCION
AS BEGIN
INSERT INTO FURIOUS_QUERYING.ITEM_X_PROMOCION(codigo_promocion,producto_id,ticket_numero,promo_aplicada_descuento, tipo_comprobante_id, ticket_fecha_y_hora, sucursal_nombre)
                            SELECT DISTINCT PROMO_CODIGO,p.id, TICKET_NUMERO, sum(PROMO_APLICADA_DESCUENTO), tp.id, TICKET_FECHA_HORA, SUCURSAL_NOMBRE
                            FROM gd_esquema.Maestra
                            JOIN FURIOUS_QUERYING.MARCA m ON m.marca = PRODUCTO_MARCA
                            JOIN FURIOUS_QUERYING.producto p ON p.nombre = PRODUCTO_NOMBRE AND p.marca_id = m.id 
							JOIN FURIOUS_QUERYING.TIPO_COMPROBANTE tp ON tp.tipo_comprobante = TICKET_TIPO_COMPROBANTE
                            AND p.subcategoria_id = PRODUCTO_SUB_CATEGORIA AND p.categoria_id = PRODUCTO_CATEGORIA                               
                            WHERE PROMO_CODIGO IS NOT NULL
							group by p.id, TICKET_NUMERO, SUCURSAL_NOMBRE, tp.id, TICKET_FECHA_HORA,PROMO_CODIGO
END
